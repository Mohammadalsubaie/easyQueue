...
//     // Use the same query criteria as the dashboard to ensure positions match
//     const q = query(
//       collection(db, "queue"),
//       where("restaurantId", "==", restaurantId),
//       where("status", "==", "Waiting"),
...
//     const unsubscribe = onSnapshot(q, (snapshot) => {
//       // Find this customer's position in the ordered queue
//       const position = snapshot.docs.findIndex(doc => doc.id === queueId) + 1;
//       setQueuePosition(position);
...
const JoinQueue = () => {
  ...
  useEffect(() => {
    ...
    // Use the same query approach as the dashboard without orderBy
    const q = query(
      collection(db, "queue"),
      where("restaurantId", "==", restaurantId),
      where("status", "==", "Waiting")
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      try {
        // Sort the docs manually by timestamp just like in the dashboard
        ...
        // Find this customer's position in the sorted queue
        const position = sortedDocs.findIndex(doc => doc.id === queueId) + 1;
        console.log("Calculated position:", position, "Queue ID:", queueId);
        setQueuePosition(position);
        ...
        setError("Failed to calculate your position");
      }
    }, (error) => {
      console.error("Position tracking error:", error);
      setError("Failed to track position");
